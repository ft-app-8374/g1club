...
export function RaceManager({ rounds }: { rounds: Round[] }) {
  ...
  return (
    <div className="space-y-3">
      {rounds.map((round) => (
        <div key={round.id} className="bg-navy-card rounded-xl border border-navy-border overflow-hidden">
          <button
            onClick={() => setExpandedRound(expandedRound === round.id ? null : round.id)}
            className="w-full flex justify-between items-center p-4 text-left hover:bg-navy-light transition"
          >
            <div>
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-400">{round.races.length} races</span>
              <span className="text-slate-500">{expandedRound === round.id ? "▲" : "▼"}</span>
            </div>
          </button>

          {expandedRound === round.id && (
            <div className="border-t border-navy-border p-4 space-y-3">
              {round.races.map((race) => (
                <div key={race.id} className="bg-navy/50 rounded-lg p-3">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center gap-2">
                      <select
                        ...
                        className="text-xs bg-navy-light border border-navy-border rounded px-2 py-1 text-slate-300"
                      >
                        ...
                        <button
                          ...
                          className={`text-xs px-2 py-0.5 rounded ${
                            runner.isScratched
                              ...
                              : "bg-navy-light text-slate-300 hover:bg-navy-border"
                          }`}
                  ...
                  {race.runners.length === 0 && (
                    <p className="text-xs text-slate-500 mt-1">No runners yet</p>
                  )}
                </div>
              ))}
              ...
              {addingRace === round.id ? (
                <div className="bg-navy/50 rounded-lg p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <input
                      ...
                      className="bg-navy-light border border-navy-border rounded px-3 py-2 text-sm text-white placeholder-slate-500 col-span-2"
                    />
                    <input
                      placeholder="Venue"
                      value={newRace.venue}
                      onChange={(e) => setNewRace({ ...newRace, venue: e.target.value })}
                      className="bg-navy-light border border-navy-border rounded px-3 py-2 text-sm text-white placeholder-slate-500"
                    />
                    <input
                    ...
                    <select
                      ...
                      className="bg-navy-light border border-navy-border rounded px-3 py-2 text-sm text-white"
                    >
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="text-sm text-slate-400 hover:text-slate-300 px-4 py-2"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <button
                  ...
                  className="w-full text-sm text-gold hover:text-gold-light border border-dashed border-navy-border rounded-lg py-2 hover:border-gold/30 transition"
                >
